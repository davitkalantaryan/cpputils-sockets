...
int socket::Tcp::connectC(const char *a_svrName, int a_port, int a_connectionTimeoutMs)
{	
...
int socket::Tcp::readC(void* a_pBuffer, int a_nSize)const
{
	return recv(m_socket, (char*)a_pBuffer, a_nSize, MSG_WAITALL);
}


int socket::Tcp::writeC(const void* a_cpBuffer, int a_nSize)
{
...
common::io::Base* socket::Tcp::Clone()const
{
	return new Tcp(*this);
}
