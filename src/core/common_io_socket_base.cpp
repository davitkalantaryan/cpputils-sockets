...
socket::Base& socket::Base::operator=(const socket::Base& a_aM)
{
...
socket::Base::operator const int&()const
{
	return m_socket;
}


socket::Base::operator int&()
{
	return m_socket;
}


void socket::Base::SetNewSocketDescriptor(int a_rawSocket)
{
	closeC();
...
int socket::Base::GetAndResetSocket()
{
	int nRet(m_socket);
...
class SocketLibInitDeinit
{
public:
	SocketLibInitDeinit();
	~SocketLibInitDeinit();
}static s_SocketLibInitDeinit;
...
socket::SocketLibInitDeinit::SocketLibInitDeinit()
{
#ifdef _WIN32
